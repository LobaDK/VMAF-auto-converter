...
for file in glob.glob(f'{input_dir}{os.path.sep}*.mp4'):
    ...
    while True:
        if attempt >= max_attempt:
            ...
            break
        attempt += 1
        p1 = subprocess.run(['ffmpeg', '-n', '-i', file, '-c:a', 'aac', '-c:v', 'libsvtav1', '-crf', str(crf_value), '-an', '-g', '600', '-preset', '8', '-pass', '1', '-f', 'null', pass_1_output])
        if p1.returncode == 0: #Skip on error or if file already exists
            p2 = subprocess.run(['ffmpeg', '-n', '-i', file, '-c:a', 'aac', '-c:v', 'libsvtav1', '-crf', str(crf_value), '-b:v', '0', '-b:a', '192k', '-g', '600', '-preset', '8', '-pass', '2', f'{output_dir}{os.path.sep}{os.path.basename(file)}'])
