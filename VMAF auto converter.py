...
logical_cores = int(os.cpu_count() / 2) # get the amount of logical cores available on system.
...
for file in glob.glob(f'{input_dir}{os.path.sep}*.{input_extension}'):
    ...
    while True:
        ...
        if not glob.glob(f'{output_dir}{os.path.sep}{os.path.basename(filename)}.*'): #check if the same filename already exists in the output folder. Extension is ignored to allow custom input container types/extensions
            ...
            subprocess.run(['ffmpeg', '-i', f'{output_dir}{os.path.sep}{os.path.basename(filename)}.{output_extension}', '-i', file, '-lavfi', f'libvmaf=log_path=log.json:log_fmt=json:n_threads={logical_cores}', '-f', 'null', '-'])
