...
def calculate(settings: dict,
              ...
              chunk_calculation_started: multiprocessing.Event,
              chunk_calculation_finished: multiprocessing.Event,
    ...
    try:
        if settings['chunk_mode'] == 1:  # GENERATE TIMINGS FOR ENCODING WITH VIDEO SPLIT INTO n EQUAL SIZED CHUNKS
            ...
            for i in range(1, settings['chunk_size'] + 1):
                ...
                # When the first chunk is calculated, set a multiprocessor event that allows chunk generation to begin
...
def generate(settings: dict,
             ...
             chunk_generator_started: multiprocessing.Event,
             chunk_generator_finished: multiprocessing.Event,
             color: str) -> None:
        ...
        while not process_failure.is_set():
            ...
            with process_lock:
                if not chunk_calculate_queue.qsize() == 0 or not chunk_calculation_finished.is_set():
                    ...
                    print(f'\n{color}Stopping {multiprocessing.current_process().name}: Nothing to do...')
                    sysexit(0)
