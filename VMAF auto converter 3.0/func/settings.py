...
def ReadSettings(log_queue: multiprocessing.Queue, manager_queue: multiprocessing.Queue) -> dict[str, str | int | float | bool]:
    """
    ...
    Returns:
        dict[str, str | int | float | bool]: The settings read from the 'settings.ini' file.
    """
    config = ConfigParser()
    ...
    for attempt in range(MAX_ATTEMPTS):
        ...
        try:
            ...
            if not settings:  # If the settings dictionary variable is empty e.g. due to the file being empty, or incorrectly being parsed, raise an exception
                raise EmptySettings('Settings file is empty or incorrectly parsed')
            break

        except KeyError as e:
            raise KeyError(f'Setting missing in settings.ini: {str(e)}')
        except (EmptySettings, KeyError) as e:
            logger.error(e)
            if attempt < MAX_ATTEMPTS - 1:
                logger.debug(f'Attempt {attempt + 1} of {MAX_ATTEMPTS} to re-create and read settings.ini')
                ...
                while EmptySettings_menu != 'Y' or EmptySettings_menu != 'N':
                    ...
                    if EmptySettings_menu == 'Y':
                        ...
                        sleep(0.1)
                        input('Press enter to continue, or make changes to the settings file and press enter when ready.')
                        break
    ...
    arguments = [
        ...
        {'names': ['-vw', '--width'], 'metavar': 'N pixels', 'dest': 'output_width', 'default': settings['output_width'], 'help': 'The desired width of the output file', 'type': int},
        {'names': ['-vh', '--height'], 'metavar': 'N pixels', 'dest': 'output_height', 'default': settings['output_height'], 'help': 'The desired height of the output file', 'type': int},
    ...
    # ArgumentTypeError is raised if one of the values is of incorrect type, and becomes a SystemExit exception.
    # We catch and send it to the manager, as an ArgumentTypeError again, so it knows not to actually log it
    # and then re-raise SystemExit to stop the script.
